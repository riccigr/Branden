...
module.exports = (app) => {
    ...
    api.findTransaction = (req, res) => {
        ...
        cache.get('payment-' + id, (cacheErr, cacheResult) => {
            if(cacheErr || !cacheResult){
                ...
                paymentDao.getById(id, (err, result) => {
                    if(err){
                        logger.info(err);
    ...
    api.create = (req, res) => {
        ...
        paymentDao.save(payment, (err, result) => {
            ...
            var cache = app.database.cache.memcachedClient();
            cache.set('payment-' + payment.id, req.body, 100000, (cacheError) => {
                if(cacheError){
            ...
            if(payment.method == "card"){
                ...
                cardClient.authorize(card, (cardError, cardReq, cardRes, cardResult) => {
                    if(cardError){
